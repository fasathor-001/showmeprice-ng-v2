...
>
> **Phase E — Jewelry & Watches subcategory applied (E.2.19.0), 2026-05-30:** new `jewelry-watches` subcategory under the existing `fashion` parent. Single data-row INSERT via `INSERT ... SELECT ... FROM categories WHERE slug='fashion'` for atomic parent-id resolution. `name='Jewelry & Watches'`, `sort_order=7` (appended after the existing 6 Fashion subs at 1-6 — no rebalance), `supports_inventory=true` (explicit, matches column default), `tier=3` (default, semantically subcategory-irrelevant), `category_features='{}'` (default). 24-element `search_aliases` JSONB populated (departure from existing subcategory pattern where aliases default to '[]' — D-143 banks this as the new standard for future subcategory additions). UK/US spelling parity: both `"jewelry"` and `"jewellery"` (Nigerian market commonly uses UK form). `"smartwatch"` deliberately EXCLUDED to avoid dual-routing ambiguity with the existing `smart-wearables` subcategory under Mobile Phones & Tablets. No `category_specs` override — parent inheritance from Fashion (size + color, both optional text) covers jewelry (ring size, chain color) and watches (case size, band color) at v1. **Seed drift acknowledgement:** the seed file's "108 total category rows" framing was the design-time inventory; actual live count was 100 pre-E.2.19.0 (some seed rows never made it to live DB, or were cleaned up over time — discrepancy not investigated here). Post-E.2.19.0 the live count is **101 categories**. No app code changes — the listings form's categories dropdown reads from DB and surfaces the new row automatically. Table count unchanged at **44**.
>
> **Phase E — Seller shop pages Step 1 applied (E.2.18.0), 2026-05-29:** `businesses.slug` (text, UNIQUE, previously nullable) **backfilled deterministically from `business_name`** via `regexp_replace + lower` SQL (4 rows at apply time after operator's pre-migration test-data cleanup → `darace-gadgets`, `jervis-luxebrand`, `reseller-by-ojemba`, `showmeprice-ng`) then **flipped to NOT NULL** in the same transaction (atomicity: no observable state with constraint enforced but rows still null). App-time inserts use a Step-2 `generateBusinessSlug()` helper that mirrors the same regex; business slugs deliberately carry NO random suffix (unlike listing slugs) because business URLs must be stable + brandable. New `business-avatars` Storage bucket created: `public=true`, `file_size_limit=2097152` (2 MB cap; avatars display at ≤80px), `allowed_mime_types=['image/jpeg', 'image/png', 'image/webp']` (excludes GIF — static branding only — and PDF). Three RLS policies on `storage.objects` mirror `product-images` shape exactly: `business_avatars_owner_insert` + `business_avatars_owner_delete` (folder match: `(storage.foldername(name))[1]` against a business owned by `auth.uid()`) + `business_avatars_public_select` (anyone reads, since avatars are public branding). `businesses.logo_path` was NOT added — already present from Phase E.1.0; this migration just enables the storage layer that will populate it. Step 2 (app code: `/sellers/[slug]` shop page, avatar upload UI on dashboard, link integration from listing detail page seller card) ships separately. Table count unchanged at **44**.
>
...
| **slug** | text | NO | — (E.2.18.0; UNIQUE. Backfilled deterministically from `business_name` via `regexp_replace + lower`; app-time inserts use `generateBusinessSlug()` — no random suffix, unlike listing slugs. Drives `/sellers/[slug]` shop page routing in Step 2.) |
...
Four buckets in Supabase Storage. All have explicit RLS policies; service role bypasses for admin signed-URL generation only.
...
### `business-avatars` (E.2.18.0)

**Public:** YES. Public-read bucket for seller business avatars / logos rendered on the `/sellers/[slug]` shop page header + listing detail seller card.
**File size limit:** 2 MB (avatars display at ≤80px; smaller cap than `product-images` because gallery-size headroom isn't needed).
**Allowed MIME types:** `image/jpeg`, `image/png`, `image/webp`. Excludes GIF (static branding only) and PDF.
**Folder structure:** `{business_id}/<filename>`. Step 2 uses timestamped filenames (e.g. `avatar-{Date.now()}.jpg`) — no `upsert:true`; replace flow inserts a new file + best-effort deletes the old, sidestepping any UPDATE-policy need.
**RLS policies (3) — mirror `product-images` exactly:**
...
- `business_avatars_public_select` — anyone can SELECT.

**Render boundary:** Step 2 adds `getBusinessAvatarPublicUrl(path)` to `src/lib/storage.ts` paralleling `getProductImagePublicUrl`. Storage path stored in `businesses.logo_path` (column existed from Phase E.1.0; no schema add).
...
**Top-level totals (seed snapshot — design-time):** 7 Tier 1 + 11 Tier 2 + 11 Tier 3 = **29 parents**. Subcategories: **80 rows** (post-E.2.19.0). Total per seed = **109 design-time rows**.

**Live count post-E.2.19.0: 101 rows** — the live DB has had ~8 fewer rows than the seed snapshot since some pre-investigation point (E.2.19.0 §0 surfaced the actual pre-migration count of 100; the discrepancy was not traced as part of E.2.19.0 scope). The taxonomy table below reflects design-time intent; refer to live DB for authoritative count.
...
| Parent | Subs | Slugs |
|---|---|---|
| Fashion & Apparel | 7 | `mens-clothing`, `womens-clothing`, `kids-clothing`, `traditional-ankara`, `shoes`, `accessories-fashion`, `jewelry-watches` (E.2.19.0) |
...
| E.2.19.0 | Feature B — Jewelry & Watches subcategory under Fashion. Single data-row INSERT via `INSERT ... SELECT ... FROM categories WHERE slug='fashion'` (atomic parent-id resolution). New row: `name='Jewelry & Watches'`, `slug='jewelry-watches'`, `sort_order=7` (appended after the 6 existing Fashion subs at 1-6), `supports_inventory=true` (explicit, matches column default), 24-element `search_aliases` JSONB with UK/US spelling parity (jewelry/jewellery + plurals + watch/watches/wristwatch/wristwatches + necklace/bracelet/earring/ring/pendant/chain/bangle/anklet singular+plural). `smartwatch` deliberately excluded to avoid dual-routing ambiguity with `smart-wearables` under Mobile Phones & Tablets. No `category_specs` override — parent inheritance from Fashion's size+color covers v1; future override is separate scope per D-143. **Seed-drift acknowledgment:** §0 surfaced live pre-migration count = 100 categories (seed snapshot framed 108 design-time rows; ~8 row discrepancy not investigated in this scope). Post-migration live count = 101. Applied 2026-05-30 via Supabase SQL Editor as `postgres` after `RESET ROLE`. §2 fully verified green across all structural (new row exists, parent_id matches Fashion via re-resolution subquery, column shape correct), data (alias_count=24, 4 critical-term spot-checks via `@>` containment all true, smartwatch absence confirmed, Fashion sub count=7, total +1), and sanity (zero duplicate slugs platform-wide, Fashion children's sort_orders array-equal `[1..7]` — catches duplicates count alone would miss). No app code changes needed — the listings form's categories dropdown reads from DB and surfaces the new row automatically. No Drizzle changes (data row, not schema). Table count unchanged at **44**. | §0 + §1 BEGIN/COMMIT + §2 structural + data + sanity (no live-fire controls — single data-row insert) (2026-05-30) |
| E.2.18.0 | Seller shop pages Step 1 — DB foundation. `businesses.slug` (text, UNIQUE, previously nullable from Phase E.1.0) backfilled deterministically from `business_name` via `regexp_replace + lower` (4 rows at apply time after operator pre-migration cleanup → `darace-gadgets`, `jervis-luxebrand`, `reseller-by-ojemba`, `showmeprice-ng`) then flipped to NOT NULL in the same transaction (atomicity — no observable state where the constraint is enforced but rows still null). New `business-avatars` Storage bucket: `public=true`, 2 MB cap, MIME allowlist `[jpeg/png/webp]`, folder structure `{business_id}/<filename>`. Three RLS policies on `storage.objects` mirroring `product-images` shape: `business_avatars_owner_insert` + `business_avatars_owner_delete` (folder match on first path segment against business owned by `auth.uid()`) + `business_avatars_public_select` (public read — avatars are public branding). `businesses.logo_path` NOT touched — already existed from Phase E.1.0; this migration enables the storage layer that populates it (per E.1.0 banking note acknowledging "the not-yet-built public storefront"). Applied 2026-05-29 via Supabase SQL Editor as `postgres` after `RESET ROLE`. §2 fully verified green: structural (slug NOT NULL + bucket config + 3 policies present), data (no NULL slugs + no collisions + deterministic slugs), defensive (businesses-table RLS unchanged, other 3 buckets unchanged, business row count preserved), and 3 live-fire controls — positive INSERT with non-null unique slug succeeded, negative INSERT with NULL slug blocked with 23502 not_null_violation, negative INSERT with duplicate slug blocked with 23505 unique_violation against `businesses_slug_unique`. Step 2 (app code: `generateBusinessSlug()` helper, `/sellers/[slug]` page, `BusinessAvatarUploader` client component, dashboard avatar widget, link integration from listing detail page seller card) ships separately. Table count unchanged at **44**. | §0 + §1 BEGIN/COMMIT + §2 structural + data + 3 live-fire controls ROLLBACK-wrapped + sanity (businesses RLS unchanged, other buckets unchanged, row count preserved) (2026-05-29) |
