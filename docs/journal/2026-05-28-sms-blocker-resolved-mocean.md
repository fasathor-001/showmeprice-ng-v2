...
- **Production state of the rejected attempt (confirmed via Supabase paste-back, 2026-05-28):** `table_exists=true, col_exists=false, check_exists=false`. The E.2.10.0 migration was **NEVER applied to production** — `businesses.seller_whatsapp` does not exist in the live DB, and the `businesses_seller_whatsapp_format` CHECK is absent. The rejected attempt exists **only as uncommitted local files**, never reached the DB. **Disposition:** these uncommitted local files are to be **discarded** before the corrected build (per D-131). Safe to discard cleanly — no DB rollback needed, no orphan column to drop, no constraint to remove.
