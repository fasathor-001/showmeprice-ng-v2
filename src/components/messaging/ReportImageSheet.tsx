...
// Stage 2.C Commit 9 — §10 report-image action sheet.
//
...
//      details textarea (≤200 chars, per reports.description CHECK) →
//      Submit
//
// Per §10.C, target_type='message' (MVP scope; per-image granularity uses
// a future enum value). Admin queue picks up the message + all its images.
...
interface ReportImageSheetProps {
  messageId: string;
  onClose: () => void;
  /** Called after a successful submit so the parent (viewer) can show toast. */
  onSubmitted: () => void;
}
